...
class SettingsController extends Controller
{
    public function edit(Request $request): Response
    {
        ...
        $configured = [
            ...
            'wppconnect' => (bool) ($s->wppconnect_base_url ?: config('services.whatsapp.wppconnect.base_url')),
        ...
        return Inertia::render('Settings/Index', [
            'settings' => $s->only([
                ...
                'wppconnect_base_url', 'wppconnect_session',
    ...
    public function update(Request $request): RedirectResponse
    {
        $data = $request->validate([
            'whatsapp_driver' => ['nullable', Rule::in(['meta', 'twilio', 'wppconnect'])],
            ...
            'wppconnect_base_url' => ['nullable', 'string', 'max:255'],
            'wppconnect_secret' => ['nullable', 'string', 'max:255'],
            'wppconnect_session' => ['nullable', 'string', 'max:64'],
            'wppconnect_webhook_secret' => ['nullable', 'string', 'max:255'],
            ...
            'elevenlabs_voice_id' => ['nullable', 'string', 'max:64', 'regex:/^[A-Za-z0-9]{15,40}$/'],
        ...
        foreach (['meta_access_token', 'twilio_auth_token', 'openai_api_key',
                  ...
                  'wppconnect_secret', 'wppconnect_webhook_secret'] as $secret) {
            if (! ($data[$secret] ?? null)) {
                unset($data[$secret]);
            }
        }
        ...
        try {
            switch ($request->provider) {
                case 'whatsapp':
                    ...
                    if ($driver->name() === 'wppconnect') {
                        /** @var WppConnectDriver $driver */
                        $status = $driver->status();
                        $connected = (bool) ($status['status'] ?? false);
                        return response()->json([
                            'ok' => $connected,
                            'message' => $connected ? 'wppconnect session connected.' : 'wppconnect session not connected. Open WhatsApp QR page to start.',
